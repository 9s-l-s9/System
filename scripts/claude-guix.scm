...
(define (usage)
  (format #t "Usage: claude-guix [MANIFEST-PATH]~%")
  ...
  (cond
    ...
    (else
     ...
     (format (current-error-port)
             "claude-guix: expected at most one manifest path argument.~%")
     (exit 1))))
...
(define (guix-args manifest)
  (append
   ...
   (maybe-mount "--expose"
                ...
                (string-append home "/.config/git/config"))
   (maybe-mount "--share"
                (string-append home "/.config/claude")
                (string-append home "/.config/claude"))
   (maybe-mount "--share"
                (string-append home "/.claude")
                (string-append home "/.claude"))
   (maybe-mount "--share"
   ...
   '("--"
     "bash" "-lc"
     "export SHELL=$(command -v bash); exec pnpm dlx @anthropic-ai/claude-code --dangerously-skip-permissions")))
...
(define (main)
  (let* ((args (cdr (command-line)))
    ...
    (unless (file-exists? manifest)
      (format (current-error-port)
              "claude-guix: manifest not found: ~a~%" manifest)
      (exit 1))

    (format #t "claude-guix: using manifest: ~a~%" manifest)
