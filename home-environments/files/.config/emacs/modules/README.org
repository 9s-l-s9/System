...
* UI
** Shackle
I prefer configuring window rules directly.
And this never really worked ;(
...
** Ebib
Ebib is really interesting but also too complicated. i am not using it enough to justify learning all the keybindings.
This made it more confusing.
Instead I want to use something more general like Recutils or my own lisp based plain text database.
...
#+end_src
** Citar
Similar to ebib. I am not using it enough
#+begin_src elisp
  ;;itar-bib liography '("~/Projects/personal-knowledge-base/main.bib"))
(use-package citar
  :custom
  (citar-bibliography '("~/Downloads/export-data(1).bib"))
  )
...
What happened to the rest of my eshell config?? lol
#+begin_src elsip
...
I am no longer really using .org files. Basically only for README files and therefore only need the basic setup.
